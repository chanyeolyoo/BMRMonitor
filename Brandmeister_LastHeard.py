    ...
    for callsign in callsigns:
        ...
        for d in data:
            if d['SourceCall'] == callsign:
                if entry.__len__() == 0 or d['Stop'] > entry['Stop'] or (d['Stop']==0 and d['Start'] > entry['Start']):
                    entry = d
    ...
    for tg in tgs:
        ...
        try:
            if history_tg[0]['Stop'] == 0:
                ...
                # text_active = STYLE_ACTIVE + '%s, %s (%ds)' % (history_tg[0]['SourceCall'], history_tg[0]['SourceName'], elapsed) + STYLE_RESET
                ...
                callsigns = [d['SourceCall'] for d in history_tg]
                text_inactive = ', '.join(callsigns)
                if len(callsigns) > 0:
        ...
        print('%s | %-30s | %s' % (text_tg, text_active, text_inactive))
